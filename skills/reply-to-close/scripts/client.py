"""Small, amount-free client for the Reply to Close gateway product."""
...
def _write_private_json(path: Path, state: dict[str, object]) -> None:
    ...
    if path.is_symlink(): raise RuntimeError("local order state is unavailable")
    temporary = path.with_name(f".{path.name}.{uuid4().hex}.tmp")
    try:
        fd = os.open(temporary, os.O_WRONLY | os.O_CREAT | os.O_EXCL, 0o600)
        with os.fdopen(fd, "w", encoding="utf-8") as file: json.dump(state, file, ensure_ascii=False, separators=(",", ":"))
...
def save_order_state(order_id: str, access_token: object, provider_data: object, access_mode: object = "paid") -> None:
    ...
    _write_private_json(_state_path(order_id), state)
...
def write_clawtip_record(provider_data: object, question: str) -> None:
    ...
    if any(not isinstance(provider_data.get(field), str) or not provider_data[field] for field in required) or provider_data.get("amount") != 6: raise RuntimeError("gateway returned an incomplete quote")
    order_no, indicator = provider_data["order_no"], provider_data["indicator"]
    if not re.fullmatch(r"[A-Za-z0-9_-]{1,32}", order_no) or not re.fullmatch(r"[0-9a-f]{32}", indicator): raise RuntimeError("gateway returned an incomplete quote")
    record = {field: provider_data[field] for field in required if field != "indicator"}; record.update({"amount": 6, "question": question})
...
def clawtip_credential(state: dict[str, object]) -> str:
    ...
    if not isinstance(provider_data, dict) or not isinstance(provider_data.get("order_no"), str) or not isinstance(provider_data.get("indicator"), str): raise RuntimeError("local ClawTip order record is unavailable")
    ...
    if not isinstance(credential, str) or not credential or len(credential) > MAX_PAYMENT_PROOF_BYTES or any(c in credential for c in "\r\n\x00"): raise RuntimeError("local ClawTip order record is unavailable")
    record.pop("payCredential", None); _write_private_json(path, record); return credential
...
def request_json(
    ...
    proof: str | None = None, access_token: str | None = None, proof_header: str = "Payment-Proof", order_id: str | None = None,
) -> dict[str, Any]:
...
def fulfill(arguments: argparse.Namespace) -> int:
    ...
    payload = request_json("POST", f"/v1/orders/{order_id}/result", proof=payment_proof, access_token=state["result_access_token"], proof_header="PAYMENT-SIGNATURE" if state["provider"] == "x402" else "Payment-Proof", order_id=order_id)
...
def parser() -> argparse.ArgumentParser:
    command = SecretFreeArgumentParser(description="Client for the Reply to Close public-test result.")
